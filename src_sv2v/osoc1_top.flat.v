...
module data_mem_wrapper (
	...
	assign csb0 = 1'b0;
	assign web0 = ~(|we_i);
	sky130_sram_1kbyte_1rw1r_32x256_8 u_physical_macro(
		.clk0(clk_i),
		.csb0(csb0),
		.web0(web0),
		.wmask0(we_i),
...
module instr_mem_wrapper (
	...
	output wire [31:0] cpu_instr_o;
	input wire host_mode_i;
	input wire host_we_i;
	input wire [31:0] host_addr_i;
	input wire [31:0] host_wdata_i;
	wire [3:0] actual_we;
	wire [31:0] actual_addr;
	wire [31:0] actual_wdata;
	assign actual_we = (host_mode_i ? {4 {host_we_i}} : 4'b0000);
	assign actual_addr = (host_mode_i ? host_addr_i : cpu_pc_i);
	assign actual_wdata = (host_mode_i ? host_wdata_i : 32'h00000000);
	sky130_sram_1kbyte_1rw1r_32x256_8 u_physical_macro(
		.clk0(clk_i),
		.csb0(1'b0),
		.web0(~(|actual_we)),
		.wmask0(actual_we),
		.addr0(actual_addr[9:2]),
		.din0(actual_wdata),
		.dout0(cpu_instr_o),
		.clk1(1'b0),
		.csb1(1'b1),
...
module osoc1_gpio (
	...
	parameter [31:0] GPIO_BASE_ADDR = 32'h80000000;
	input wire clk_i;
	input wire rst_ni;
	...
	output wire [3:0] sram_we_o;
	input wire [31:0] sram_rdata_i;
	input wire [31:0] gpio_i;
	output reg [31:0] gpio_o;
	wire is_gpio_addr;
	wire is_gpio_write;
	assign is_gpio_addr = cpu_addr_i == GPIO_BASE_ADDR;
	assign is_gpio_write = is_gpio_addr & (cpu_we_i != 4'b0000);
	always @(posedge clk_i or negedge rst_ni)
		if (!rst_ni)
			gpio_o <= 32'h00000000;
		else if (is_gpio_write)
			gpio_o <= cpu_wdata_i;
	assign sram_we_o = (is_gpio_addr ? 4'b0000 : cpu_we_i);
	assign cpu_rdata_o = (is_gpio_addr ? gpio_i : sram_rdata_i);
endmodule
module osoc1_top (
	clk_i,
	rst_ni,
	...
	parameter signed [31:0] IMEM_DEPTH = 256;
	parameter signed [31:0] DMEM_DEPTH = 256;
	parameter [31:0] GPIO_BASE_ADDR = 32'h80000000;
	input wire clk_i;
	input wire rst_ni;
	input wire host_mode_i;
	...
	wire core_rst_ni;
	wire [31:0] core_pc_to_imem;
	wire [31:0] imem_instr_to_core;
	wire [3:0] core_dmem_we;
	wire [31:0] core_dmem_addr;
	wire [31:0] core_dmem_wdata;
	wire [31:0] dmem_rdata_to_core;
	wire [3:0] sram_we;
	wire [31:0] sram_rdata;
	assign core_rst_ni = rst_ni & ~host_mode_i;
	...
	instr_mem_wrapper #(.DEPTH(IMEM_DEPTH)) u_instr_mem(
		.clk_i(clk_i),
		.cpu_pc_i(core_pc_to_imem),
		.cpu_instr_o(imem_instr_to_core),
		.host_mode_i(host_mode_i),
		.host_we_i(host_we_i),
		.host_addr_i(host_addr_i),
		.host_wdata_i(host_wdata_i)
	);
	osoc1_gpio #(.GPIO_BASE_ADDR(GPIO_BASE_ADDR)) u_gpio(
		.clk_i(clk_i),
		.rst_ni(rst_ni),
		.cpu_we_i(core_dmem_we),
		.cpu_addr_i(core_dmem_addr),
		.cpu_wdata_i(core_dmem_wdata),
		.cpu_rdata_o(dmem_rdata_to_core),
		.sram_we_o(sram_we),
